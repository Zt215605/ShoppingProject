<template>
  <div class="detail">
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <!-- 这里其实直接遍历就行了，这样写复杂了，就连v-show都可以不用书写，因为后面就一个插值 -->
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <!-- 这里的数据是请求服务器得到的数据，所以数据没有回来之前渲染会出问题，至少应该带一个空数组过去 -->
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:"
                  class="mins"
                  @click="skuNum > 0 ? skuNum-- : (skuNum = 0)"
                  >-</a
                >
              </div>
              <div class="add">
                <!-- 这里再加入购物车，进行路由跳转之前，发请求把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      let result;
      try {
        result = await this.$store.dispatch("addOrUpdateShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        // 成功  路由跳转传递参数
        // 一些简单的参数skuNum，通过query的形式给路由组件传递过去
        // 产品信息的数据比较复杂，通常直接通过会话存储（不持久化，会话结束数据消失）
        // 不管是本地存储还是会话存储，一般都是存储字符串
    ...
    /*  其实这里还可以使用常规的处理方式，回调函数的方式，但是有async和await谁还用这个，这个感觉太低端了
        addShopcar() {
