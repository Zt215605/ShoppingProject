...
export default {
  ...
  methods: {
    ...
    async getPayInfo() {
      let result;
      try {
        result = await this.$API.reqPayInfo(this.orderId);
        console.log('erweima',result);
        if (result.code == 200) {
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, "请你微信支付", {
        ...
        beforeClose: (type, instance, done) => {
          ...
          if (type == "cancel") {
            ...
            // 判断是否真的支付了
            // if (this.code == 200) {    //这里先注释了，为了开发人员不付钱方便
            clearInterval(this.timer);
            this.timer = null;
            done();
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          try {
            let result = await this.$API.reqPayStates(this.orderId);
            if (result.code == 200) {
              // 清除定时器
              clearInterval(timer);
              this.timer = null;
